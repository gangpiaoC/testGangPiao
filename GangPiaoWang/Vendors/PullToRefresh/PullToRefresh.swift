...
private var xoTag: String = "com.pullToRefresh.footerRefresh"
extension UIScrollView {
    var footerRefresh: UIView {
        get {
            return (objc_getAssociatedObject(self, &xoTag) as! UIView)
        }
        set {
            objc_setAssociatedObject(self, &xoTag, newValue, .OBJC_ASSOCIATION_RETAIN)
        }
    }
}
...
public extension UIScrollView{
    ...
    public func setUpFooterRefresh<T:UIView>(_ footer:T,action:@escaping ()->())->T where T:RefreshableFooter{
        ...
        containComponent.isHidden = true
        self.footerRefresh = containComponent
        return footer
    }
